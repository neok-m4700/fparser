...
class Use_Stmt(StmtBase):  # R1109
    ...
    def match(string):
        ...
        i = line.find(',')
        if i == -1:
            return nature, Module_Name(line), '', None
        name = line[:i].rstrip()
        if not name:
            return
...
for clsname in _names:
    cls = eval(clsname)
    if not (isinstance(cls, ClassType) and issubclass(cls, Base) and not cls.__name__.endswith('Base')):
        continue
